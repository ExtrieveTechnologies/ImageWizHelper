...
public ImageWizHelperJNI() throws Exception 
{
	...
	if (Files.exists(Paths.get(dllPath))) {
		try
		{
			...
			if(Files.exists(Paths.get(licPath))) {
			synchronized (lock) {
				if(!unlockLibrary(readLicData(licPath),error_info)) {//*****Here we are calling "unlockLibrary" function to Unlock our libraries and use its functionalities*****
					if (error_info.length() > 0)					//***** Calling this "unlockLibrary" function is important before initializing***** 
...
private static native boolean unlockLibrary(String license_file_content, StringBuffer error_info);
